...
Binds 127.0.0.1 only. The Chrome extension talks to it over HTTP:
  ...
  POST /update                    upgrade yt-dlp now (restarts when idle)
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        try:
            ...
            if attempt < max_attempts:
                # A leftover .part from an earlier failed run makes yt-dlp resume
                # with a Range request, and a freshly signed URL rejects that
                # outright -- the attempt then dies in seconds having moved no
                # bytes at all. Distinguish the two cases: nothing transferred
                # means the resume offset is the problem, so start the file over;
                # if bytes did move, resuming is working and worth keeping.
                opts["continuedl"] = should_resume_after_failure(attempt_saw_data[0])
                if not attempt_saw_data[0]:
                    log("nothing transferred -- discarding the partial file "
                        "and starting this download over")
                streams.clear()
...
def reveal_in_explorer(path):
    if sys.platform == "win32":
        # Pass the path as a real argument -- string interpolation into a shell
        # command breaks on quotes and is an injection risk.
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/version":
            with UPDATE_STATE_LOCK:
                self._send_json(200, dict(UPDATE_STATE))
            return
        ...
        if path == "/update":
            with UPDATE_STATE_LOCK:
                if UPDATE_STATE["checking"]:
                    ...
                    return
            threading.Thread(target=check_for_ytdlp_update,
                             kwargs={"restart_on_update": True}, daemon=True).start()
...
def warm_up_ytdlp():
    ...
    """
    try:
        yt_dlp.YoutubeDL({"quiet": True, "no_warnings": True}).close()
    except Exception as e:
...
def main():
    ...
    try:
        ...
        print(f"Stop it first, or set YTDLP_SERVER_PORT to a free port.", file=sys.stderr)
        return 1
